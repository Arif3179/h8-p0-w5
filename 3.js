...
function questLogger(quests, actions) {
 ...
 for(i=0;i<quests.length;i++){
  output.push({})
  output[i].name = quests[i]['name']
   quests[i]['name'] = []
   for(j=0;j<quests[i]['requirements'].length;j++){
     for(k=0;k<actions.length;k++){
     let flag = false
     if(quests[i]['requirements'][j] == actions[k]){
       flag = true
       quests[i]['name'].push(flag)
      } 
     
     }
  } if(quests[i]['name'].length == quests[i]['requirements'].length){
    output[i].status = 'completed'
  } else if(quests[i]['name'].length < quests[i]['requirements'].length && quests[i]['name'].length!= 0 ){
    output[i].status = 'on progress'
  } else if(quests[i]['name'].length == 0){
    output[i].status = 'failed'
  }
 }
return output
}
